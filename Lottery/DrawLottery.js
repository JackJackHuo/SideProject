...
function drawTicket(){
  ...
  while(ticketArray.length < 2){
    alphaPick = Math.floor(Math.random()*letter.length)
...
let numberArray =[]
let newNumber
while(numberArray.length < players.length){  
  do{
    newNumber = drawTicket()   
  }
  while(numberArray.includes(newNumber))
  numberArray.push(newNumber)
}

//分配彩票號碼給買家
for(let i = 0 ; i < players.length ; i++){
   players[i].number = numberArray[i]
}
